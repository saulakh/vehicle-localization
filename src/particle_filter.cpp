...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for (unsigned int i = 0; i < observations.size(); i++) {
    ...
    for (unsigned int j = 0; j < predicted.size(); j++) {
      ...
      // If closest so far, update min_dist and closest_id
  ...
  for (int p=0; p < num_particles; p++) {
    ...
    for (unsigned int i = 0; i < observations.size(); i++) {
      ...
      for (unsigned int j = 0; j < map_landmarks.landmark_list.size(); j++) {
        ...
        //double current_dist = dist(particles[p].x, particles[p].y, landmark_x, landmark_y);
        
        // If landmark is within sensor range
        if (current_dist < sensor_range) {
          int association = landmark_id;
          
          // Add to predictions
          //predictions.push_back(LandmarkObs{ landmark_id, landmark_x, landmark_y });
          
          // Find closest landmark
          //ParticleFilter::dataAssociation(predictions, transformed_obs);
          //int association = transformed_obs[i].id;
